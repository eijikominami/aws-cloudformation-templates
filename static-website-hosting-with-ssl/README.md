...
#### Amazon Kinesis Firehose and related resouces

``Amazon Kinesis Data Firehose`` delivers the logs to ``Amazon S3`` and ``Amazon Elasticsearch Service``.
Kinesis Firehose invoke an ``AWS Lambda`` function to process logs, and update the log format.
Kinesis Firehose sends logs to ``Amazon S3`` bucket where it is unable to deliver the data to Elasticsearch. 
