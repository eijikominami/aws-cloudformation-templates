...
- AWS Organizations setup (for organization-wide security services)
- Control Tower deployment (if using Control Tower integration)
- Appropriate IAM permissions for security services (GuardDuty, Security Hub, Config, etc.)
...
This template sets up ``AWS Security Agent``, an AI-powered penetration testing service. It creates the prerequisite infrastructure — IAM service role, IAM app role for console access, S3 artifact bucket, CloudWatch log group, scanner security group, and an SNS topic for pentest completion notifications — and then provisions an **Agent Space** via a Lambda-backed Custom Resource.

After deploying this stack, complete the Security Agent setup in the console:
1. Register and verify a **Target Domain** for your web application.
2. Create a **Penetration Test** configuration referencing the outputs `AgentSpaceId` and `ScannerSecurityGroupId`.
3. Optionally link a GitHub repository integration for automatic code remediation PRs.
