...
このテンプレートは、AI を活用したペネトレーションテストサービスである ``AWS Security Agent`` のセットアップを行います。IAM サービスロール、コンソールアクセス用 IAM アプリロール、S3 アーティファクトバケット、CloudWatch ロググループ、スキャナー用セキュリティグループ、およびペンテスト完了通知用 SNS トピックを作成し、Lambda バックドの Custom Resource を使用して **エージェントスペース** をプロビジョニングします。

スタックのデプロイ後、コンソールで以下の追加設定を行ってください：
1. Web アプリケーションの **ターゲットドメイン** を登録・検証する。
2. 出力値 `AgentSpaceId` と `ScannerSecurityGroupId` を参照して **ペネトレーションテスト** 設定を作成する。
3. 必要に応じて GitHub リポジトリを連携し、自動コード修正 PR を有効にする。
