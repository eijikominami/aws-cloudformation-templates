...
| GovernedRegions | CommaDelimitedList | | | このアカウントの管理対象リージョンのリスト（空の場合は全リージョン） |
...
> [!NOTE]
> **CloudWatch.2（不正な API コール）:** 不正な API コールの詳細検知は CloudWatch Logs Subscription Filter（[Notification](../notification) スタックの `analyzeUnauthorizedApiCalls` Lambda）で行っており、通知に呼び出し元の情報を含めています。メトリクスフィルタでは呼び出し元の詳細が取得できないため、閾値を 10件/分（Sum）に変更しバースト検知用として残しています（Security Hub 準拠維持のため）。